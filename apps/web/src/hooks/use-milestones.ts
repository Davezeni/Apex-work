...
export interface Milestone {
  id: string; orderId: string; title: string; description: string | null;
  amountEtb: number; dueDate: string | null; position: number;
  status: 'PENDING' | 'DELIVERED' | 'APPROVED' | 'DISPUTED';
  deliveredAt: string | null; approvedAt: string | null;
...
export function useSetMilestones(orderId: string | undefined) {
  ...
  return useMutation<{ items: Milestone[] }, Error, MilestoneInput[]>({
    mutationFn: (milestones) => apiFetch(`/orders/${orderId}/milestones`, {
      method: 'PUT', token, body: { milestones },
    }),
    onSuccess: () => {
...
export function useMilestoneAction(orderId: string | undefined) {
  const token = useAuthStore((s) => s.accessToken);
  const qc = useQueryClient();
  return useMutation<Milestone, Error, { id: string; action: 'deliver' | 'approve' | 'dispute'; reason?: string }>({
    mutationFn: ({ id, action, reason }) =>
      apiFetch(`/milestones/${id}/${action}`, { method: 'POST', token, body: reason ? { reason } : undefined }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['milestones', orderId] });
      qc.invalidateQueries({ queryKey: ['order', orderId] });
