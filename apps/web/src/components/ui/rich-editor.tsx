...
import { Bold, Italic, List, ListOrdered, Link as LinkIcon, Undo2, Redo2, Heading2, Quote, Code } from 'lucide-react';
 ...
 */
export function RichEditor({ value, onChange, placeholder, className, minRows = 6, maxChars }: Props) {
  ...
  return (
    <div className={cn('rounded-2xl border border-border bg-card overflow-hidden focus-within:border-primary focus-within:ring-4 focus-within:ring-primary/20', className)}>
...
function Toolbar({ editor }: { editor: Editor }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1 border-b border-border bg-background/60 px-2 py-1">
      ...
      <button type="button" onClick={() => editor.chain().focus().toggleHeading({ level: 2 }).run()} className={btn(editor.isActive('heading', { level: 2 }))} aria-label="Heading"><Heading2 className="h-4 w-4" /></button>
      ...
      <button type="button" onClick={() => editor.chain().focus().toggleCodeBlock().run()} className={btn(editor.isActive('codeBlock'))} aria-label="Code"><Code className="h-4 w-4" /></button>
      <span className="mx-1 h-4 w-px bg-border" />
      <button type="button" onClick={promptLink} className={btn(editor.isActive('link'))} aria-label="Link"><LinkIcon className="h-4 w-4" /></button>
      <span className="mx-1 h-4 w-px bg-border" />
