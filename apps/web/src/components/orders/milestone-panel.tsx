...
import { useMilestones, useSetMilestones, useMilestoneAction, type Milestone } from '@/hooks/use-milestones';
...
export function MilestonePanel({ orderId, amountEtb, isClient, isSeller, orderStatus }: Props) {
  ...
  const [drafts, setDrafts] = useState<{ title: string; amount: string; dueDate: string; description: string }[]>([]);

  useEffect(() => {
    if (planning && drafts.length === 0) {
      // Seed with a single milestone equal to the whole order.
      setDrafts([{ title: 'Full delivery', amount: String(amountEtb), dueDate: '', description: '' }]);
    }
  }, [planning, drafts.length, amountEtb]);

  const canPlan =
    isClient && (orderStatus === 'PENDING' || orderStatus === 'ACTIVE') &&
  ...
  const submitPlan = async () => {
    if (drafts.some((d) => !d.title.trim() || !d.amount)) return toast.error('Fill every milestone');
    const sum = drafts.reduce((a, d) => a + Number(d.amount || 0), 0);
    if (sum !== amountEtb) return toast.error(`Amounts must sum to ${formatEtb(amountEtb)} — got ${formatEtb(sum)}`);
    try {
      await setPlan.mutateAsync(drafts.map((d) => ({
        title: d.title.trim(),
  ...
  if (isLoading) {
    return <div className="grid h-20 place-items-center"><Loader2 className="h-4 w-4 animate-spin text-muted-foreground" /></div>;
  }
  ...
  return (
    <section className="rounded-2xl border border-border bg-card p-4">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-extrabold uppercase tracking-widest text-muted-foreground">Milestones</h3>
          {items.length > 0 && (
      ...
      {items.length > 0 && (
        <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-muted">
          <div className="grad-hero h-full transition-all" style={{ width: `${(paid / (total || 1)) * 100}%` }} />
        </div>
      )}
      ...
      {planning && (
        <div className="mt-3 space-y-3">
          {drafts.map((d, i) => (
            <div key={i} className="rounded-xl border border-dashed border-primary/30 bg-primary/5 p-3">
              <div className="flex items-center gap-2">
                <div className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-primary text-[10px] font-bold text-white">{i + 1}</div>
                <input
                  value={d.title}
                ...
                />
                {drafts.length > 1 && (
                  <button onClick={() => setDrafts(drafts.filter((_, j) => j !== i))} aria-label="Remove" className="grid h-7 w-7 place-items-center rounded-full text-red-500">
              ...
              <div className="mt-2 grid grid-cols-2 gap-2">
                <input
                  value={d.amount}
                  onChange={(e) => setDrafts(drafts.map((x, j) => j === i ? { ...x, amount: e.target.value.replace(/[^0-9]/g, '') } : x))}
                  inputMode="numeric"
                ...
                <input
                  type="date"
                  value={d.dueDate}
                  onChange={(e) => setDrafts(drafts.map((x, j) => j === i ? { ...x, dueDate: e.target.value } : x))}
                  className="rounded-lg border border-border bg-background px-2 py-1.5 text-sm outline-none focus:border-primary"
                />
              </div>
              <textarea
                ...
                rows={2}
                placeholder="What's delivered? (optional)"
          ...
          <div className="flex gap-2">
            <Button
              ...
              onClick={() => setDrafts([...drafts, { title: '', amount: '', dueDate: '', description: '' }])}
              disabled={drafts.length >= 20}
            >
              <Plus className="h-3.5 w-3.5" /> Add
            </Button>
            <Button size="sm" variant="outline" className="flex-1" onClick={() => { setPlanning(false); setDrafts([]); }}>
              Cancel
            </Button>
            <Button size="sm" variant="brand" className="flex-1" onClick={submitPlan} disabled={setPlan.isPending}>
              {setPlan.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : 'Save'}
            </Button>
          </div>
          <p className="text-center text-[11px] text-muted-foreground">
            Must sum to {formatEtb(amountEtb)} · current: {formatEtb(drafts.reduce((a, d) => a + Number(d.amount || 0), 0))}
          </p>
        </div>
      )}
    </section>
...
function MilestoneRow({
  index, m, busy, canDeliver, canApprove, canDispute, onDeliver, onApprove, onDispute,
}: {
  index: number; m: Milestone; busy: boolean;
  canDeliver: boolean; canApprove: boolean; canDispute: boolean;
  onDeliver: () => void; onApprove: () => void; onDispute: () => void;
}) {
  const statusMeta = {
    PENDING:   { icon: <Clock className="h-3.5 w-3.5" />, cls: 'bg-muted text-muted-foreground', label: 'Pending' },
    DELIVERED: { icon: <Send className="h-3.5 w-3.5" />, cls: 'bg-amber-500/10 text-amber-500', label: 'Delivered' },
    APPROVED:  { icon: <CheckCircle2 className="h-3.5 w-3.5" />, cls: 'bg-emerald-500/10 text-emerald-500', label: 'Paid' },
    DISPUTED:  { icon: <AlertCircle className="h-3.5 w-3.5" />, cls: 'bg-red-500/10 text-red-500', label: 'Disputed' },
  }[m.status];
  return (
    <div className={cn('rounded-xl border p-3', m.status === 'APPROVED' ? 'border-emerald-500/30' : 'border-border')}>
      <div className="flex items-start gap-2">
        <div className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-muted text-[10px] font-bold">{index}</div>
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 text-[11px]">
            ...
            {m.dueDate && <span className="text-muted-foreground"> · due {new Date(m.dueDate).toLocaleDateString()}</span>}
          </div>
