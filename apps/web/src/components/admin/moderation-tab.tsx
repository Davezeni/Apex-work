...
export function ModerationTab() {
  ...
  const scan = useMutation({
    mutationFn: () =>
      apiFetch<{
        scanned: number;
        flagged: number;
        items: { id: string; kind: string; title: string; summary: string }[];
      }>('/admin/ops/moderation/scan', { method: 'POST', token }),
    onSuccess: (r) => {
  ...
  return (
    <div className="space-y-5">
      <SectionHead
        title={dt('Moderation')}
        subtitle={dt('Review and act on gigs, jobs and reviews')}
        actions={
          <button
            onClick={() => scan.mutate()}
            disabled={scan.isPending}
            className="inline-flex items-center gap-1.5 rounded-full border border-border bg-card px-3 py-1.5 text-[11px] font-bold text-muted-foreground transition-colors hover:text-foreground disabled:opacity-50"
          >
...
function FlaggedTriage() {
  ...
  const bulk = useMutation({
    mutationFn: (st: string) =>
      apiFetch(`/admin/ops/flagged/bulk`, {
        method: 'POST',
        token,
        body: { ids: [...sel], status: st },
      }),
    onSuccess: (r: any, st: string) => {
      toast.success(`Bulk ${st === 'RESOLVED' ? 'resolved' : 'dismissed'} ${r.updated}`);
      setSel(new Set());
      qc.invalidateQueries({ queryKey: ['admin/flagged'] });
    },
    onError: (e) => toast.error((e as Error).message),
  ...
  return (
    <div className="space-y-3">
      ...
      {isLoading ? (
        ...
        <Empty message="No flagged gigs in this state" />
      ) : (
        <div className="space-y-2">
          <label className="flex items-center gap-2 text-xs font-bold text-muted-foreground">
            <input
              ...
              checked={allSel}
              onChange={(e) =>
                setSel(e.target.checked ? new Set(items.map((i) => i.id)) : new Set())
              }
            />
            Select all
          </label>
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-[11px] text-muted-foreground">
                    @{g.owner?.fullName} · {g.viewsCount} views · {g.ordersCount} orders
                  </div>
                  {g.flaggedReason && (
                    <p className="mt-1 text-xs text-amber-600">{g.flaggedReason}</p>
                  )}
                  {g.moderatorNotes && (
                    <p className="mt-1 text-[11px] italic text-muted-foreground">
                      Notes: {g.moderatorNotes}
                    </p>
                  )}
                  <div className="mt-2 flex flex-wrap gap-2">
                    {status !== 'IN_REVIEW' && (
                      <Button
                        ...
                        onClick={() => triage.mutate({ id: g.id, body: { status: 'IN_REVIEW' } })}
                      >
                        {dt('Start review')}
                      </Button>
                    )}
                    <Button
                      ...
                      variant="brand"
                      disabled={triage.isPending}
                      onClick={() => triage.mutate({ id: g.id, body: { status: 'RESOLVED' } })}
                    >
                      {dt('Resolve')}
                    </Button>
                    <Button
                      ...
                      variant="ghost"
                      disabled={triage.isPending}
                      onClick={() => triage.mutate({ id: g.id, body: { status: 'DISMISSED' } })}
                    >
                      {dt('Dismiss')}
                    </Button>
                    {status === 'QUEUED' && (
                      <Button
                        ...
                        onClick={() =>
                          triage.mutate({
                            id: g.id,
                            body: {
                              assignee: prompt('Assignee username (leave blank to self?)') ?? '',
                            },
                          })
                        }
                      >
                ...
                <Badge tone={mStatusTone[g.moderationStatus] ?? 'neutral'}>
                  {g.moderationStatus}
                </Badge>
              </div>
            </div>
...
function GigsModeration() {
  ...
  const { data, isLoading } = useQuery<any>({
    ...
    queryFn: () =>
      apiFetch(`/admin/ops/gigs?status=${status}&${flaggedOnly ? 'flagged=1' : ''}&limit=25`, {
        token,
      }),
  });
  ...
  return (
    <div className="space-y-3">
      ...
      {isLoading ? (
        <Spinner label={dt('Loading gigs…')} />
      ) : items.length === 0 ? (
        <Empty message="No gigs found" />
      ) : (
        <TableShell>
          <thead>
            <tr>
              ...
              <tr key={g.id} className="border-b border-border/50">
                ...
                <Td>
                  <div className="flex items-center gap-1.5">
                    ...
                    {g.isFlagged && <Badge tone="bad">{dt('flagged')}</Badge>}
                  </div>
                </Td>
                <Td>
                  <div className="flex flex-wrap gap-1.5">
                    {g.status === 'ACTIVE' && (
                      <Button
                        ...
                        onClick={() => act.mutate({ id: g.id, body: { status: 'PAUSED' } })}
                      >
                        {dt('Pause')}
                      </Button>
                    )}
                    ...
                    <Button
                      size="sm"
                      variant={g.isFlagged ? 'outline' : 'destructive'}
                      disabled={act.isPending}
                      onClick={() =>
                        act.mutate({
                          id: g.id,
                          body: {
                            ...
                            flaggedReason: g.isFlagged ? null : 'Admin flag',
                          },
                        })
                      }
                    >
...
function JobsModeration() {
  ...
  const { data, isLoading } = useQuery<any>({
    ...
    queryFn: () =>
      apiFetch(`/admin/ops/jobs?limit=25${open ? `&open=${open === '1' ? '1' : '0'}` : ''}`, {
        token,
      }),
  });
  const act = useMutation({
  ...
  return (
    <div className="space-y-3">
      <select
        ...
        className={cn(inputCls, 'w-auto')}
      >
        <option value="">{dt('All jobs')}</option>
        <option value="1">{dt('Open')}</option>
        <option value="0">{dt('Closed')}</option>
      </select>
      {isLoading ? (
        <Spinner label={dt('Loading jobs…')} />
      ) : items.length === 0 ? (
          ...
          <tbody>
            {items.map((j) => (
              <tr key={j.id} className="border-b border-border/50">
                <Td className="max-w-[240px]">
                  <div className="truncate font-bold">{j.title}</div>
                </Td>
                <Td className="text-muted-foreground">@{j.client?.username}</Td>
                <Td className="text-muted-foreground">
                  {j.budgetMinEtb != null
                    ? `${formatEtb(j.budgetMinEtb)}–${formatEtb(j.budgetMaxEtb)}`
                    : '—'}
                </Td>
                <Td>
                  <Badge tone={j.isOpen ? 'ok' : 'neutral'}>{j.isOpen ? 'Open' : 'Closed'}</Badge>
                </Td>
                <Td>
                  <div className="flex gap-1.5">
                    {j.isOpen ? (
                      <Button
                        ...
                        variant="destructive"
                        disabled={act.isPending}
                        onClick={() => act.mutate({ id: j.id, body: { isOpen: false } })}
                      >
                        {dt('Close')}
                      </Button>
                    ) : (
                      <Button
                        ...
                        variant="brand"
                        disabled={act.isPending}
                        onClick={() => act.mutate({ id: j.id, body: { isOpen: true } })}
                      >
                        {dt('Reopen')}
                      </Button>
                    )}
                    <Button
                      ...
                      variant="outline"
                      disabled={act.isPending}
                      onClick={() => act.mutate({ id: j.id, body: { pinned: !j.pinnedAt } })}
                    >
                      {j.pinnedAt ? 'Unpin' : 'Pin'}
                    </Button>
                  </div>
                </Td>
              </tr>
...
function ReviewsModeration() {
  ...
  const act = useMutation({
    mutationFn: (vars: { id: string; action: 'HIDE' | 'RESTORE'; reason?: string }) =>
      apiFetch(`/admin/ops/reviews/${vars.id}/moderate`, {
        method: 'POST',
        token,
        body: { action: vars.action, reason: vars.reason },
      }),
    onSuccess: () => {
      toast.success(dt('Review updated'));
  ...
  return (
    <div className="space-y-3">
      ...
      {isLoading ? (
        <Spinner label={dt('Loading reviews…')} />
      ) : items.length === 0 ? (
          ...
          <tbody>
            {items.map((r) => (
              <tr key={r.id} className="border-b border-border/50">
                ...
                <Td>
                  <Badge tone={r.hiddenAt ? 'bad' : 'ok'}>{r.hiddenAt ? 'Hidden' : 'Live'}</Badge>
                </Td>
                <Td>
                  {r.hiddenAt ? (
                    <Button
                      ...
                      variant="brand"
                      disabled={act.isPending}
                      onClick={() => act.mutate({ id: r.id, action: 'RESTORE' })}
                    >
                      {dt('Restore')}
                    </Button>
                  ) : (
                    <Button
                      ...
                      variant="destructive"
                      disabled={act.isPending}
                      onClick={() => act.mutate({ id: r.id, action: 'HIDE', reason: 'Admin hide' })}
                    >
                      {dt('Hide')}
                    </Button>
                  )}
