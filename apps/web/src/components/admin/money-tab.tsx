...
export function MoneyTab() {
  ...
  return (
    <div className="space-y-5">
      <SectionHead
        title={dt('Orders & Money')}
        subtitle={dt('Orders, refunds, wallet ledger and payouts')}
      />
      <div className="flex gap-1 rounded-xl bg-muted p-1">
        {(['orders', 'ledger', 'withdrawals', 'reconcile', 'health'] as Sub[]).map((s) => (
          <button
            ...
            onClick={() => setSub(s)}
            className={cn(
              'flex-1 rounded-lg px-3 py-2 text-sm font-bold capitalize transition-colors',
              sub === s ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground',
            )}
          >
            {s}
          </button>
...
function Orders() {
  ...
  const { data, isLoading } = useQuery<any>({
    ...
    queryFn: () =>
      apiFetch(
        `/admin/ops/orders?limit=25${status ? `&status=${status}` : ''}${q ? `&q=${encodeURIComponent(q)}` : ''}`,
        { token },
      ),
  });
  const refundMut = useMutation({
    mutationFn: () =>
      apiFetch(`/admin/ops/orders/${refund!.id}/refund`, {
        method: 'POST',
        token,
        body: { amountEtb: Number(amount), reason },
      }),
    onSuccess: () => {
      toast.success(dt('Refund issued'));
      setRefund(null);
      setAmount('');
      setReason('');
    ...
    <div className="space-y-3">
            ...
            {items.map((o) => (
              <tr key={o.id} className="border-b border-border/50">
                <Td>
                  ...
                  <div className="text-[11px] text-muted-foreground">{o.orderNumber}</div>
                </Td>
                ...
                <Td>
                  <Button
                    ...
                    onClick={() => setRefund({ id: o.id, title: o.title })}
                  >
                    {dt('Refund')}
                  </Button>
                </Td>
      ...
      {refund && (
        ...
        >
          ...
          >
            <h3 className="text-base font-extrabold">{dt('Refund order')}</h3>
            <p className="text-xs text-muted-foreground">{refund.title}</p>
            <Field label={dt('Amount (ETB)')}>
              <input
                type="number"
                min={1}
                value={amount}
            ...
            <div className="flex justify-end gap-2">
              <Button variant="ghost" size="sm" onClick={() => setRefund(null)}>
                {dt('Cancel')}
              </Button>
              <Button
                variant="destructive"
...
function Ledger() {
  ...
  const { data, isLoading } = useQuery<any>({
    ...
    queryFn: () =>
      apiFetch(`/admin/ops/ledger?limit=30${userId ? `&userId=${userId}` : ''}`, { token }),
  });
  const adjust = useMutation({
    mutationFn: () =>
      apiFetch(`/admin/ops/wallet/${userId}/adjust`, {
        method: 'POST',
        token,
        body: { type, amountEtb: Number(amt), description: desc },
      }),
    onSuccess: () => {
      toast.success(dt('Wallet adjusted'));
      setAdj(false);
      setAmt('');
      setDesc('');
      qc.invalidateQueries({ queryKey: ['admin/ledger'] });
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-end gap-2">
        ...
        <Button size="sm" variant="brand" onClick={() => setAdj((v) => !v)}>
          {adj ? 'Close' : 'Adjust wallet'}
        </Button>
      </div>
      {adj && (
        <div className="space-y-3 rounded-xl border border-border bg-muted/40 p-4">
          ...
          <input
            placeholder={dt('Reason (required, audited)')}
            value={desc}
          ...
          <Button
            size="sm"
            variant="brand"
            disabled={!userId || !amt || !desc || adjust.isPending}
      ...
      {isLoading ? (
        <Spinner label={dt('Loading ledger…')} />
      ) : items.length === 0 ? (
            ...
            {items.map((t) => (
              <tr key={t.id} className="border-b border-border/50">
                ...
                <Td
                  className={cn(
                    'font-semibold',
                    t.amountEtb >= 0 ? 'text-emerald-600' : 'text-red-600',
                  )}
                >
                  {t.amountEtb >= 0 ? '+' : ''}
                  {formatEtb(t.amountEtb)}
                </Td>
...
function Withdrawals() {
  ...
  const { data, isLoading } = useQuery<any>({
    ...
    queryFn: () =>
      apiFetch(`/admin/ops/withdrawals?limit=25${status ? `&status=${status}` : ''}`, { token }),
  });
  const act = useMutation({
    mutationFn: (vars: { id: string; status: string }) =>
      apiFetch(`/admin/ops/withdrawals/${vars.id}/status`, {
        method: 'POST',
        token,
        body: { status: vars.status },
      }),
    onSuccess: () => {
      toast.success(dt('Withdrawal updated'));
      qc.invalidateQueries({ queryKey: ['admin/withdrawals-ops'] });
    },
    onError: (e) => toast.error((e as Error).message),
  });
  const items: any[] = data?.items ?? [];
  const tone: Record<string, 'ok' | 'warn' | 'neutral' | 'bad'> = {
    SUCCESS: 'ok',
    PROCESSING: 'warn',
    PENDING: 'neutral',
    FAILED: 'bad',
  ...
  return (
    <div className="space-y-3">
      ...
      {isLoading ? (
        <Spinner label={dt('Loading withdrawals…')} />
      ) : items.length === 0 ? (
        <Empty message="No withdrawals" />
      ) : (
        <TableShell>
          <thead>
            <tr>
                ...
                <Td>
                  {w.status === 'PENDING' && (
                    <Button
                      ...
                      onClick={() => act.mutate({ id: w.id, status: 'PROCESSING' })}
                    >
                      {dt('Process')}
                    </Button>
                  )}
                  {w.status === 'PROCESSING' && (
                    <>
                      <Button
                        ...
                        variant="brand"
                        disabled={act.isPending}
                        onClick={() => act.mutate({ id: w.id, status: 'SUCCESS' })}
                      >
                        {dt('Mark success')}
                      </Button>
                      <Button
                        ...
                        variant="destructive"
                        disabled={act.isPending}
                        onClick={() => act.mutate({ id: w.id, status: 'FAILED' })}
                      >
                        {dt('Fail')}
                      </Button>
                    </>
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <p className="text-xs text-muted-foreground">
          Verifies every wallet balance equals the signed sum of its ledger transactions.
        </p>
        <button
          onClick={() => refetch()}
          disabled={isLoading}
          className="inline-flex shrink-0 items-center gap-1.5 rounded-full border border-border bg-card px-3 py-1.5 text-[11px] font-bold text-muted-foreground hover:text-foreground disabled:opacity-50"
        >
          {dt('Recheck')}
        </button>
      </div>
      {isLoading ? (
        <Spinner label={dt('Reconciling…')} />
      ) : (
        <>
          <div className="grid grid-cols-3 gap-2">
            <Mini tx={data?.wallets ?? 0} label={dt('Wallets')} />
            <Mini tx={data?.reconciled ?? 0} label={dt('In sync')} />
            <Mini tx={data?.drifted ?? 0} label={dt('Drifted')} warn={(data?.drifted ?? 0) > 0} />
          </div>
          {data && data.netDriftEtb !== 0 && (
            <div className="rounded-xl border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-xs font-semibold text-amber-600">
              Net ledger vs wallet drift: {formatEtb(data.netDriftEtb)}
            </div>
          )}
          {drifts.length === 0 ? (
            <Empty message="All wallets reconcile ✓" />
          ) : (
            <TableShell>
              <thead>
                <tr>
              ...
              <tbody>
                {drifts.slice(0, 100).map((r) => (
                  <tr key={r.userId} className="border-b border-border/50">
                    ...
                    <Td>
                      <Badge tone="bad">{formatEtb(r.driftEtb)}</Badge>
                    </Td>
                  </tr>
                ))}
              </tbody>
...
function OrderHealth() {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <p className="text-xs text-muted-foreground">
          Orders breaching an SLA or stuck in the flow. Highest severity first.
        </p>
        <div className="flex shrink-0 gap-1.5">
          <button
            onClick={() => refetch()}
            disabled={isLoading}
            className="inline-flex items-center gap-1.5 rounded-full border border-border bg-card px-3 py-1.5 text-[11px] font-bold text-muted-foreground hover:text-foreground disabled:opacity-50"
          >
            {dt('Refresh')}
          </button>
          <button
          ...
          >
            {dt('Send daily digest')}
          </button>
        </div>
      </div>
      {isLoading ? (
        <Spinner label={dt('Checking order health…')} />
      ) : (
        <>
          <div className="grid grid-cols-3 gap-2">
            <Mini tx={data?.total ?? 0} label={dt('Attention')} warn={(data?.total ?? 0) > 0} />
            <Mini tx={data?.high ?? 0} label={dt('High')} warn={(data?.high ?? 0) > 0} />
            <Mini tx={(data?.medium ?? 0) + (data?.low ?? 0)} label={dt('Med + Low')} />
          </div>
          <div className="flex flex-wrap gap-1.5">
            {Object.entries(data?.counts ?? {}).map(([k, v]) => (
              <span
                key={k}
                className="rounded-full border border-border bg-card px-2.5 py-1 text-[11px] font-bold text-muted-foreground"
              >
                {catLabel[k] ?? k}: {v}
              </span>
            ))}
          ...
          {(data?.inbox ?? []).length === 0 ? (
            <Empty message="No orders currently need attention ✓" />
          ) : (
            <TableShell>
              <thead>
                <tr>
