...
export function UsersImportButton() {
  ...
  const mutate = useMutation({
    mutationFn: (text: string) => {
      ...
      }>('/admin/ops/users/import', { method: 'POST', token, body: { rows } });
    },
    onSuccess: (r) => {
        ...
        >
          ...
          >
            <div className="flex items-center justify-between">
              <h3 className="text-base font-extrabold">{dt('Bulk import users')}</h3>
              <button
                onClick={() => setOpen(false)}
                aria-label={dt('Close')}
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              ...
              <code className="font-mono">
                {dt('fullName, username, phone, email, role, password')}
              </code>
              . role is CLIENT/FREELANCER/ADMIN (optional).
            </p>
            <div className="mt-3 flex items-center gap-2">
              ...
              />
              <span className="text-[11px] text-muted-foreground">{dt('or paste below')}</span>
            </div>
            <textarea
              value={csv}
            ...
            <div className="mt-3 flex justify-end gap-2">
              ...
              <Button
                size="sm"
                variant="brand"
                onClick={() => mutate.mutate(csv)}
                disabled={mutate.isPending || !csv.trim()}
              >
                {mutate.isPending
                  ? 'Importing…'
                  : `Create users${csv.split('\n').filter(Boolean).length ? ` (${csv.split('\n').filter(Boolean).length} rows)` : ''}`}
              </Button>
            </div>
          </div>
        </div>
