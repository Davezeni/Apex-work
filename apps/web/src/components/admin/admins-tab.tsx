...
export function AdminsTab() {
  ...
  const roleMut = useMutation({
    ...
    onSuccess: () => { toast.success('Role updated'); qc.invalidateQueries({ queryKey: ['admin/admins'] }); qc.invalidateQueries({ queryKey: ['admin/staff-search'] }); },
  ...
  const candidates: any[] = (results?.items ?? []).filter((u: any) => !STAFF_ROLES.includes(u.role));
  // Keep the results list fresh after a promotion.
  useEffect(() => { qc.invalidateQueries({ queryKey: ['admin/staff-search', query] }); }, [query, qc]);
  ...
  return (
    <div className="space-y-4">
      <SectionHead title="Admin team" subtitle="Staff roles & permissions (RBAC)" actions={
        <Button size="sm" variant="brand" onClick={() => setAddOpen((v) => !v)}><UserPlus className="mr-1 h-4 w-4" /> Add staff</Button>
      } />

      {addOpen && (
        <div className="space-y-2 rounded-2xl border border-border bg-card p-4">
          <div className="text-sm font-extrabold">Add a staff member</div>
          <div className="flex flex-wrap gap-2">
            <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search by username, phone or email…" className={cn(inputCls, 'min-w-0 flex-1')} />
            <select value={role} onChange={(e) => setRole(e.target.value)} className={cn(inputCls, 'w-auto')}>
              {STAFF_ROLES.map((r) => <option key={r} value={r}>{r}</option>)}
            </select>
          </div>
          <div className="text-[11px] text-muted-foreground">Pick a user below and assign a staff role. The chosen role determines their capabilities.</div>
          {isFetching ? <Spinner label="Searching…" /> : query.trim().length >= 2 && candidates.length === 0 ? <Empty message="No eligible users found (they may already be staff)" /> : null}
          {candidates.length > 0 && (
              ...
              {candidates.slice(0, 8).map((u) => (
                <div key={u.id} className="flex items-center justify-between gap-2 p-2.5">
                  <div className="min-w-0">
                    <div className="truncate text-sm font-bold">{u.fullName} <span className="text-muted-foreground">@{u.username}</span></div>
                    <div className="text-[11px] text-muted-foreground">{u.email ?? u.phone ?? '—'} · {u.role}</div>
                  </div>
                  <Button size="sm" variant="brand" disabled={roleMut.isPending} onClick={() => roleMut.mutate({ userId: u.id, role })}>
                    {roleMut.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : `Make ${role}`}
                  </Button>
                </div>
              ))}
            </div>
