...
interface Msg { role: 'user' | 'assistant'; content: string; source?: 'ai' | 'fallback' }
...
export function AIAssistant() {
  ...
  const persist = (next: Msg[]) => {
    setMsgs(next);
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify(next.slice(-MAX_HISTORY))); }
    catch { /* ignore */ }
  };
  ...
  const send = async () => {
    ...
    try {
      ...
      persist([...next, {
        role: 'assistant',
        content: (err as { message?: string }).message ?? 'Something went wrong. Please try again.',
        source: 'fallback',
      }]);
    }
  };
        ...
        {open && (
          <>
            ...
            <motion.div
              ...
              className="safe-bottom fixed inset-x-3 bottom-40 z-40 mx-auto flex max-h-[70dvh] max-w-md flex-col overflow-hidden rounded-3xl border border-border bg-card shadow-2xl sm:right-4 sm:left-auto sm:w-96"
            >
              <header className="grad-hero flex items-center gap-2 p-3 text-white">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-[10px] opacity-80">Ask anything about Apex-Work</div>
                </div>
                {msgs.length > 0 && (
                  <button onClick={clear} className="text-[10px] font-bold uppercase tracking-wider opacity-80 active:opacity-100">
                    Clear
                  </button>
                )}
              </header>
                ...
                {msgs.map((m, i) => (
                  <div key={i} className={cn('flex', m.role === 'user' ? 'justify-end' : 'justify-start')}>
                    <div className={cn(
                      ...
                      m.role === 'user' ? 'grad-hero rounded-br-md text-white' : 'rounded-bl-md bg-muted text-foreground',
                    )}>
                      {m.content}
                      {m.role === 'assistant' && m.source === 'fallback' && (
                        <div className="mt-1 text-[10px] opacity-70">⚠ Offline reply</div>
                      )}
                  ...
                  <div className="flex justify-start">
                    <div className="rounded-2xl rounded-bl-md bg-muted px-3 py-2 text-sm text-muted-foreground">
                      <span className="inline-flex gap-1">
                        ...
                        <span className="h-2 w-2 animate-bounce rounded-full bg-primary" style={{ animationDelay: '.15s' }} />
                        <span className="h-2 w-2 animate-bounce rounded-full bg-primary" style={{ animationDelay: '.3s' }} />
                      </span>
                    </div>
                  </div>
                )}
              ...
              <div className="border-t border-border p-2">
                <div className="flex items-end gap-2">
                  ...
                  <button
                    ...
                    aria-label="Send"
                    className="grad-hero grid h-9 w-9 shrink-0 place-items-center rounded-full text-white disabled:opacity-40 active:scale-90"
                  >
