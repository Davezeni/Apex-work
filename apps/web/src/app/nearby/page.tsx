        ...
        <header className="safe-top sticky top-0 z-10 border-b border-border bg-background/95 px-3 py-3 backdrop-blur-xl">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <h1 className="text-lg font-extrabold tracking-tight">Nearby freelancers</h1>
        ...
        <div className="mx-3 mt-4 space-y-2">
          {items.length === 0 && !isLoading && (
            <div className="rounded-2xl border border-dashed border-border p-6 text-center">
              ...
              <p className="mt-2 text-sm font-semibold">No freelancers here yet</p>
              ...
              {me?.role === 'FREELANCER' && (
                <Button asChild size="sm" variant="brand" className="mt-3">
                  <Link href="/settings/profile">Add my location</Link>
                </Button>
              )}
            </div>
          )}
          {items.map((f) => (
            <Link key={f.id} href={`/u/${f.username}`} className="flex items-center gap-3 rounded-2xl border border-border bg-card p-3">
              {f.avatarUrl ? (
                <Image src={f.avatarUrl} alt={f.fullName} width={40} height={40} unoptimized className="h-10 w-10 rounded-full object-cover" />
              ) : (
              ...
              <div className="text-right text-[11px]">
                ...
                {f.ratingCount > 0 && (
                  <div className="text-muted-foreground">
                    <Star className="mr-0.5 inline h-2.5 w-2.5 fill-amber-400 text-amber-400" />{f.rating.toFixed(1)}
                  </div>
                )}
              </div>
            </Link>
