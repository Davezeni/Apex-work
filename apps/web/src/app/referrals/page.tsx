...
export default function ReferralsPage() {
  ...
  const trackClick = (source: string) => {
    ...
    void apiFetch('/referrals/track', { method: 'POST', body: { refCode: code, source } }).catch(
      () => undefined,
    );
  ...
  const copy = async () => {
    try {
      ...
      trackClick('link');
      toast.success(dt('Referral link copied'));
    } catch {
    ...
    if (typeof navigator !== 'undefined' && navigator.share) {
      try {
        await navigator.share({
          title: 'Join Apex-Work',
          text: "Join me on Apex-Work — Ethiopia's freelance marketplace.",
          url: link,
        });
        trackClick('share');
      } catch {
  ...
  return (
    <div className="min-h-dvh bg-background pb-24">
      <header className="safe-top sticky top-0 z-10 flex items-center gap-3 border-b border-border bg-background/95 px-3 py-3 backdrop-blur-xl">
        ...
        <h1 className="text-lg font-extrabold tracking-tight">{dt('Referrals')}</h1>
      </header>
      ...
      <section className="mx-3 mt-4 rounded-2xl border border-border bg-card p-4">
        <div className="grad-hero overflow-hidden rounded-2xl p-5 text-white shadow-lg shadow-primary/30">
          ...
          <div className="mt-3 text-2xl font-extrabold tracking-tight">
            Earn 100 ETB per friend 🎉
          </div>
          <p className="mt-1 text-xs text-white/80">
            Share your link — when a friend completes their first order, you both get a bonus.
          </p>
          <div className="mt-4 rounded-xl bg-white/15 px-3 py-2 font-mono text-xs backdrop-blur">
            {code}
          </div>
        </div>
      </section>
      ...
      <section className="mx-3 mt-4 grid grid-cols-2 gap-2">
        <Stat
          ...
          label={dt('Referred')}
          value={String(ref?.stats.total ?? 0)}
        />
        <Stat
          icon={<Sparkles className="h-4 w-4" />}
          label={dt('Active')}
          value={String(ref?.stats.active ?? 0)}
        />
        <Stat
          icon={<Gift className="h-4 w-4" />}
          label={dt('Earned')}
          value={formatEtb(ref?.stats.commissionEtb ?? 0)}
        />
        <Stat
          icon={<MousePointerClick className="h-4 w-4" />}
          label={dt('Link clicks')}
          value={String(ref?.linkClicks ?? 0)}
        />
      </section>

      {ref && ref.referred.length > 0 && (
        <section className="mx-3 mt-4 rounded-2xl border border-border bg-card p-4">
          <div className="text-sm font-bold">{dt('Your referrals')}</div>
          <div className="mt-2 space-y-2">
            {ref.referred.slice(0, 20).map((r) => (
              <div key={r.userId} className="flex items-center justify-between gap-2 text-xs">
                <div className="min-w-0 truncate font-semibold">
                  {r.fullName} <span className="text-muted-foreground">@{r.username}</span>
                </div>
                <div className="shrink-0 text-right">
                  <div className="font-bold">{formatEtb(r.spentEtb)}</div>
                  <div className="text-muted-foreground">
                    {r.completedOrders > 0 ? `${r.completedOrders} order(s)` : 'no orders yet'}
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="mx-3 mt-4 rounded-2xl border border-border bg-card p-4">
        <div className="text-sm font-bold">{dt('How it works')}</div>
        <ol className="mt-3 space-y-3 text-xs">
          <Step
            n={1}
            title={dt('Share your link')}
            desc="Send it to friends who need freelance help or want to earn."
          />
          <Step
            n={2}
            title={dt('They sign up')}
            desc="Your friend creates an account using your link."
          />
          <Step
            n={3}
            title={dt('You both earn')}
            desc="When they complete their first order, 100 ETB is added to each wallet."
          />
        </ol>
      </section>
    </div>
...
function Stat({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="rounded-xl border border-border bg-card p-3 text-center">
      <div className="mx-auto grid h-8 w-8 place-items-center rounded-full bg-primary/10 text-primary">
        {icon}
      </div>
