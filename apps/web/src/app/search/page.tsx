      ...
      <div className="min-h-dvh bg-background pb-24">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder={dt('Search gigs, jobs, people…')}
        ...
        {q.trim().length < 2 && (
          <div className="mx-4 mt-12 text-center">
            ...
            <p className="mt-4 text-sm font-semibold">{dt('Search the whole marketplace')}</p>
            <p className="mt-1 text-xs text-muted-foreground">
              Type at least 2 letters — try &ldquo;react&rdquo;, &ldquo;amharic&rdquo;,
              &ldquo;logo&rdquo;.
            </p>
          </div>
        )}
        ...
        {q.trim().length >= 2 && data && !anyResults && (
          <div className="mx-4 mt-8 rounded-2xl border border-dashed border-border p-8 text-center">
            ...
            <p className="mt-3 text-sm font-semibold">{dt('No matches')}</p>
            <p className="mt-1 text-xs text-muted-foreground">
              {dt('Try a different spelling, fewer words, or a broader term.')}
            </p>
        ...
        {data && anyResults && (
          <div className="mx-3 mt-4 space-y-6">
            {(tab === 'all' || tab === 'gigs') && data.gigs.length > 0 && (
              <ResultGroup title={dt('Gigs')} icon={<PackageIcon className="h-3 w-3" />}>
                <div className="grid grid-cols-2 gap-2">
                  {data.gigs.map((g) => (
                    <Link
                      key={g.id}
                    ...
                    >
                      ...
                      <div className="mt-1 flex items-center gap-1 text-[10px] text-muted-foreground">
                        ...
                        {g.rating.toFixed(1)} · {formatEtb(g.startingPriceEtb)}+
                      </div>
                    </Link>
                  ))}
                </div>
            ...
            {(tab === 'all' || tab === 'jobs') && data.jobs.length > 0 && (
              <ResultGroup title={dt('Jobs')} icon={<Briefcase className="h-3 w-3" />}>
                <div className="space-y-2">
                  {data.jobs.map((j) => (
                    <Link
                      key={j.id}
            ...
            {(tab === 'all' || tab === 'users') && data.users.length > 0 && (
              <ResultGroup title={dt('People')} icon={<UserIcon className="h-3 w-3" />}>
                <div className="space-y-2">
                  {data.users.map((u) => (
                    <div
                      key={u.id}
                    ...
                    >
                      ...
                      {me && me.id !== u.id && (
                        <button
                          ...
                          className="grid h-10 w-10 shrink-0 place-items-center rounded-full border border-border text-muted-foreground transition-transform hover:text-primary active:scale-90"
                        >
