  ...
  if (isLoading || !item) {
    return (
      <div className="grid min-h-dvh place-items-center bg-background">
        {error ? (
          <p className="text-sm text-destructive">{dt('Not found')}</p>
        ) : (
      ...
      <header className="safe-top absolute inset-x-0 top-0 z-10 flex items-center justify-between p-3">
        ...
        {item.externalUrl && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-1 rounded-full bg-black/50 px-3 py-2 text-xs font-bold text-white backdrop-blur"
          >
      ...
      >
        {isImageType('', item.imageUrl) ? (
          ...
          <div className="flex flex-col items-center gap-3 text-white">
            ...
            <span className="text-sm font-semibold">{dt('Open video')}</span>
          </div>
        ) : (
          <div className="flex flex-col items-center gap-3 text-white">
        ...
        <Link href={`/u/${item.owner.username}`} className="mt-3 flex items-center gap-2">
          ...
          <div className="text-xs">
            ...
            <div className="text-muted-foreground">
              {item.owner.title ?? '@' + item.owner.username}
            </div>
          </div>
