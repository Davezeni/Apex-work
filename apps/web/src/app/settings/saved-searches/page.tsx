...
import {
  useSavedSearches, useUpdateSavedSearch, useDeleteSavedSearch,
  ...
  return (
    <div className="min-h-dvh bg-background pb-24">
      <header className="safe-top sticky top-0 z-10 flex items-center gap-3 border-b border-border bg-background/95 px-3 py-3 backdrop-blur-xl">
        ...
        <div>
          <h1 className="text-lg font-extrabold tracking-tight">Saved searches</h1>
          <div className="text-[10px] text-muted-foreground">Get pinged when new matches appear</div>
        </div>
      </header>
      ...
      {items.length === 0 && (
        <div className="mx-4 mt-8 rounded-2xl border border-dashed border-border p-8 text-center">
          ...
          <p className="mt-3 text-sm font-semibold">No saved searches yet</p>
          <p className="mt-1 text-xs text-muted-foreground">
            Run a search anywhere in Apex-Work, tap &ldquo;Save&rdquo;, and we&rsquo;ll
            notify you when fresh matches show up.
          </p>
            ...
            <div key={s.id} className="rounded-2xl border border-border bg-card p-3">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <Link href={url} className="text-sm font-bold hover:underline">{s.name}</Link>
                ...
                <button
                  onClick={() => update.mutate({ id: s.id, pushEnabled: !s.pushEnabled, emailEnabled: !s.pushEnabled })}
                ...
                >
                  {s.pushEnabled ? <Bell className="h-4 w-4 text-primary" /> : <BellOff className="h-4 w-4" />}
                </button>
                <button
                  onClick={() => { if (window.confirm('Delete this saved search?')) remove.mutate(s.id, { onSuccess: () => toast.success('Deleted') }); }}
                  aria-label="Delete"
