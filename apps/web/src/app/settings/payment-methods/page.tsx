  ...
  return (
    <div className="min-h-dvh bg-background pb-24">
      <header className="safe-top sticky top-0 z-10 flex items-center gap-3 border-b border-border bg-background/95 px-3 py-3 backdrop-blur-xl">
        ...
        <div>
          <h1 className="text-lg font-extrabold tracking-tight">{dt('Payment methods')}</h1>
          <p className="text-[11px] text-muted-foreground">{dt('Secure checkout through Chapa')}</p>
        </div>
      </header>
      ...
      <section className="mx-3 mt-6 rounded-2xl border border-border bg-card p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-sm font-bold">{dt('How your payment is protected')}</h2>
            <ol className="mt-2 space-y-2 text-xs leading-relaxed text-muted-foreground">
              <li>
                <b className="text-foreground">1.</b> Apex-Work creates a unique order reference.
              </li>
              <li>
                <b className="text-foreground">2.</b> Chapa hosts the wallet, bank, or card payment
                form.
              </li>
              <li>
                <b className="text-foreground">3.</b> Apex-Work verifies the transaction server-side
                before activating the order.
              </li>
              <li>
                <b className="text-foreground">4.</b> Funds stay in escrow until delivery is
                accepted or a dispute is resolved.
              </li>
            </ol>
          </div>
        </div>
      ...
      <div className="mx-3 mt-5">
        <Button
          ...
          size="lg"
          className="w-full"
          disabled={config.data?.enabled === false}
        >
          <Link href="/browse">
