...
export default function SupportPage() {
  ...
  const submit = async () => {
    if (subject.trim().length < 4) return toast.error('Add a short subject');
    if (body.trim().length < 10) return toast.error('Describe the problem');
    try {
      const t = await create.mutateAsync({ subject: subject.trim(), category, body: body.trim() });
      setOpening(false); setSubject(''); setBody(''); setCategory('general');
  ...
  return (
    <div className="min-h-dvh bg-background pb-24">
      <header className="safe-top sticky top-0 z-10 flex items-center gap-3 border-b border-border bg-background/95 px-3 py-3 backdrop-blur-xl">
        ...
        <h1 className="text-lg font-extrabold tracking-tight">Support</h1>
      ...
      {opening && (
        <section className="mx-3 mt-4 rounded-2xl border border-primary/30 bg-primary/5 p-4">
          <div className="text-sm font-bold">Open a ticket</div>
          <input
            value={subject} onChange={(e) => setSubject(e.target.value)} maxLength={140}
            placeholder="Subject" className="mt-2 w-full rounded-xl border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary" />
          <div className="mt-2 flex flex-wrap gap-1">
            {(['general', 'billing', 'dispute', 'tech'] as const).map((c) => (
              <button key={c} onClick={() => setCategory(c)}
                className={cn('rounded-full px-2.5 py-1 text-[11px] font-bold capitalize',
                  category === c ? 'bg-primary text-white' : 'bg-card text-muted-foreground border border-border')}>
                {c}
              </button>
            ))}
          </div>
          <textarea
            value={body} onChange={(e) => setBody(e.target.value)} rows={4} maxLength={4000}
          ...
          <div className="mt-2 flex gap-2">
            <Button variant="outline" size="default" className="flex-1" onClick={() => setOpening(false)}>Cancel</Button>
            <Button variant="brand" size="default" className="flex-1" onClick={submit} disabled={create.isPending}>
      ...
      {items.length === 0 && !opening && (
        <div className="mx-4 mt-8 rounded-2xl border border-dashed border-border p-8 text-center">
          ...
          <p className="mt-3 text-sm font-semibold">No tickets yet</p>
          <p className="mt-1 text-xs text-muted-foreground">Open one for anything the AI assistant can&rsquo;t resolve.</p>
        </div>
      )}
...
function TicketRow({ t }: { t: SupportTicket }) {
  const meta =
    ...
    t.status === 'WAITING_STAFF' ? 'bg-amber-500/10 text-amber-500' :
    t.status === 'WAITING_USER' ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground';
  return (
    <Link href={`/support/${t.id}`} className="flex items-center gap-3 rounded-2xl border border-border bg-card p-3">
