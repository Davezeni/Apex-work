  ...
  return (
    <MobileShell activeTab="chat">
      <div className="flex min-h-[85dvh] flex-col items-center justify-center px-6 text-center">
        ...
        <h1 className="mt-5 text-2xl font-extrabold">Group invite</h1>
        <p className="mt-2 max-w-sm text-sm text-muted-foreground">
          {joined
            ? 'You’re in! Opening the conversation…'
            : 'You’ve been invited to join a group on Apex-Work.'}
        </p>
        {!isAuthed ? (
          <div className="mt-6 flex gap-3">
            <Button asChild variant="brand" size="lg"><a href="/login">Sign in to join</a></Button>
          </div>
        ) : joined ? (
          ...
          <Button onClick={doJoin} disabled={join.isPending} variant="brand" size="lg" className="mt-6">
            {join.isPending ? <Loader2 className="h-5 w-5 animate-spin" /> : 'Join group'}
          </Button>
        )}
      </div>
    </MobileShell>
