...
interface Picked { id: string; fullName: string; username: string; avatarUrl: string | null }
...
export default function NewGroupPage() {
  ...
  const toggle = (u: Picked) => {
    setPicked((p) => p.some((x) => x.id === u.id) ? p.filter((x) => x.id !== u.id) : [...p, u].slice(0, 50));
  };

  const submit = async () => {
    if (title.trim().length < 2) return toast.error('Group needs a name');
  ...
  return (
    <div className="min-h-dvh bg-background pb-32">
      <header className="safe-top sticky top-0 z-10 flex items-center gap-3 border-b border-border bg-background/95 px-3 py-3 backdrop-blur-xl">
        ...
        <h1 className="text-lg font-extrabold tracking-tight">New group</h1>
      </header>

      <div className="mx-3 mt-4 space-y-3">
        <div>
          <label className="mb-1 block text-xs font-semibold uppercase tracking-wider text-muted-foreground">Group name</label>
          <input
            value={title}
            onChange={(e) => setTitle(e.target.value)}
            maxLength={80}
            placeholder="Project · Team · Anything"
        ...
        {picked.length > 0 && (
          <div className="rounded-2xl border border-border bg-card p-3">
            ...
            <div className="flex flex-wrap gap-1.5">
              {picked.map((p) => (
                <button key={p.id} onClick={() => toggle(p)} className="inline-flex items-center gap-1 rounded-full bg-primary/10 pl-1 pr-2 py-0.5 text-xs">
                  {p.avatarUrl ? (
                    <Image src={p.avatarUrl} alt={p.fullName} width={20} height={20} unoptimized className="h-5 w-5 rounded-full object-cover" />
                  ) : (
        ...
        <div className="space-y-1">
          {results.map((u) => {
            ...
            return (
              ...
              >
                {u.avatarUrl ? (
                  <Image src={u.avatarUrl} alt={u.fullName} width={36} height={36} unoptimized className="h-9 w-9 rounded-full object-cover" />
                ) : (
          ...
          {q.trim().length < 2 && (
            <p className="mt-6 text-center text-xs text-muted-foreground">Type a name or @username to find people.</p>
          )}
        </div>
      </div>
      ...
      <div className="safe-bottom fixed inset-x-0 bottom-0 z-20 border-t border-border bg-background/95 px-4 pb-4 pt-3 backdrop-blur-xl">
        <Button variant="brand" size="lg" className="w-full" onClick={submit} disabled={create.isPending || picked.length === 0 || title.trim().length < 2}>
          {create.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : `Create group (${picked.length})`}
        </Button>
      </div>
    </div>
  );
