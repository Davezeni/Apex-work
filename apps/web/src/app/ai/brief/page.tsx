...
export default function AIBriefPage() {
  ...
  const [result, setResult] = useState<{ title: string; description: string; skills: string[]; budgetMinEtb: number; budgetMaxEtb: number; source: 'ai' | 'fallback' } | null>(null);
  const ai = useAIBrief();
  ...
  return (
    <div className="min-h-dvh bg-background pb-24">
      ...
      <div className="mx-3 mt-4">
        ...
        />
        <Button variant="brand" size="lg" className="mt-3 w-full" onClick={generate} disabled={ai.isPending}>
          {ai.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <><Sparkles className="h-4 w-4" /> Generate job brief</>}
        </Button>
      </div>
